...
class CNN(nn.Module):
    ...
    def forward(self, x):
        ...
        # batchsize, channel, seq_len, embed_size
        x = output.view(self.batch, 1, -1, self.input_size)
        # batchsize, output_channel, (length-3+1, 1) feature map
        x1 = F.relu(self.conv3(x))
        # batchsize, output_channel, (length-4+1, 1) feature map
        x2 = F.relu(self.conv4(x))
        # batchsize, output_channel, (length-5+1, 1) feature map
